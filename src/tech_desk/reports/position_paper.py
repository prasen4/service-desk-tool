...
research brief didn't supply any."""
    ...
    def _render_docx(self, vendor_name: str, paper: dict) -> Path:
        ...
        # Shared Cotiviti brand chrome (cover page, header/footer, fonts,
        # table styling) lives in docx_theme so Position Papers and
        # Technology Desk Reports render with an identical look.
        doc = theme.new_branded_document(title=vendor_name, subtitle="Position Paper")

        def add_heading(text: str, level: int = 1) -> None:
            theme.add_heading(doc, text, level=level)

        def add_bullets(items) -> None:
            theme.add_bullets(doc, items)

        def add_body(text: str) -> None:
            theme.add_body(doc, text)

        def set_cell(cell, text: str, *, header: bool = False, center: bool = False) -> None:
            theme.set_cell(cell, text, header=header, center=center)
